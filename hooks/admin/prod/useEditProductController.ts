...
import { webUrl } from "@/src/config/env";
...
export function useEditProductController(id: number) {

  const { token } = useAuth();
  const idx = Number(useLocalSearchParams().id ?? 0);
  ...
  const [initialCatid, setInitialCatid] = useState<number | null>(null);
  const [initialSubcatid, setInitialSubcatid] = useState<number | null>(null);
...
const fetchRow = useCallback(async () => {
  try {
    const response = await laraapi.get(`/admin/prod/edit/${idx}`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    const { row, cats, subcats } = response.data;
    setRow(row);
    setCats(cats);
    setSubcats(subcats);
    ...
    if (row.img) setExistingLargeImagePath(row.img);
    if (row.img2) setExistingThumbImagePath(row.img2);
    if (row.filer) setExistingFileName(row.filer);


    const resolvedCatid = row.catid || null;
    const resolvedSubcatid = row.subcatid || null;

    setCatid(resolvedCatid);
    setSubcatid(resolvedSubcatid);
    setInitialCatid(resolvedCatid);
    setInitialSubcatid(resolvedSubcatid);

  } catch (err) {
    console.error("Failed to fetch product:", err);
    setError("⚠️ Unable to load product");
  }
}, [idx, token]);
...
async function checkProductNameExists(name: string, catid: number | null, subcatid: number | null, token: string): Promise<boolean> {
  if (!name.trim() || !catid || !subcatid || !idx) return false;

  try {
    const response = await laraapi.post(
      "/admin/prod/checkedit",
      { id: idx, name: name.trim(), catid, subcatid },
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
async function checkProductCodeExists(coder: string, token: string): Promise<boolean> {
  if (!coder.trim() || !idx) return false;

  try {
    const response = await laraapi.post(
      "/admin/prod/checkcodeedit",
      { id: idx, coder: coder.trim() },
      { headers: { Authorization: `Bearer ${token}` } }
    );
    return response.data.exists === true;
  } catch (err) {
...
if (file) {
  if (Platform.OS === "web") {
    ...
    try {
        const response = await laraapi.post(`/admin/prod/update/${idx}`, formData, {
          headers: {
